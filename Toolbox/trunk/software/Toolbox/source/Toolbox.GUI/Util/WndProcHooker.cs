...
namespace Toolbox.GUI.Util
{
  public class WndProcHooker
  {
    ...
    public static void HookControlRecursive(  Control ctl, WndProcCallback callback, uint msg )
    {
      WndProcHooker.HookWndProc(ctl, callback, msg);

      foreach (Control ctl2 in ctl.Controls)
        WndProcHooker.HookWndProc(ctl2, callback, msg);
    }
        ...
        IntPtr hwnd, uint msg, uint wParam, int lParam)
    {
      if (hwndDict.ContainsKey(hwnd))
      {
        ...
        else if (hpi.messageMap.ContainsKey( 0) )
        {
          WndProcCallback callback = hpi.messageMap[0];
          bool handled = false;
          int retval = callback(hwnd, msg, wParam, lParam, ref handled);
          if (handled)
            return retval;
