...
namespace AT.Toolbox
{
  ...
  public class SynchronizeOperationWrapper : IOperationWrapper
  {
    ...
    public SynchronizeOperationWrapper(ISynchronizeInvoke invoker)
    {
      if (invoker == null)
    ...
    public TType Invoke<TType>(Func<TType> action)
    {
      ...
      if (m_invoker.InvokeRequired)
        return (TType)m_invoker.Invoke(action, new object[0]);
      else
        return action();
    }
    ...
    public void Invoke(Action action)
    {
      ...
      if (m_invoker.InvokeRequired)
      {
        if (this.CallProceduresAsync)
          m_invoker.BeginInvoke(action, new object[0]);
        else
          m_invoker.Invoke(action, new object[0]);
      }
      else
        action();
    }
