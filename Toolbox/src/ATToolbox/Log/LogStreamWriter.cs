...
namespace AT.Toolbox.Log
{
  ...
  public class LogStreamWriter : TextWriter
  {
    ...
    /// <param name="info">Тип сообщений. True - Info, False - Debug</param>
    public LogStreamWriter(string source, bool info)
    {
    ...
    public override void Flush()
    {
      lock (m_buffer)
      {
        if (m_buffer.Count > 0)
        {
          string description = m_buffer.Count > 1 ? string.Join(this.NewLine, m_buffer.ToArray()) : "";
