...
namespace AT.Toolbox.MSSQL.DAL.RecordBinding.Wcf
{
  public abstract class ExceptionHandlingProxyBase<T> : ICommunicationObject, IDisposable
    where T : class
  {
    private static readonly log4net.ILog Log = log4net.LogManager.GetLogger(typeof(ExceptionHandlingProxyBase<T>).Name);
    ...
    public ExceptionHandlingProxyBase(string endpointConfigurationName, string hostName, int basePort)
    {
      if (this.m_isInitialized) throw new InvalidOperationException("Object already initialized.");
      this.m_isInitialized = true;
      ...
      var uri = m_channelFactory.Endpoint.Address.Uri;
      string newAddress = string.Format("{0}://{1}:{2}{3}", uri.Scheme, hostName, basePort + uri.Port, uri.PathAndQuery);
      m_channelFactory.Endpoint.Address = new EndpointAddress(newAddress);
    }
    ...
    protected virtual void Cleanup()
    {
      ...
      {
        Log.Error("Cleanup(): exception", ex);
        ...
        {
          Log.Error("Cleanup(): exception 2", ex2);
        }
      }
      ...
      {
        Log.Error("Cleanup(): exception 3", ex);
        ...
        {
          Log.Error("Cleanup(): exception 4", ex);
        }
      }
    ...
    protected void Invoke(string operationName, params object[] parameters)
    {
      ...
      {
        Log.Error(string.Format("Invoke({0}): exception", operationName), targetEx);
        ...
        {
          Log.Error(string.Format("Invoke({0}): exception 2", operationName), targetEx2);
    ...
    protected TResult Invoke<TResult>(MethodInfo methodInfo, params object[] parameters)
    {
      ...
      {
        Log.Error(string.Format("Invoke<{0}>({1}): exception", typeof(TResult), methodInfo), targetEx);
        CommunicationException commEx = targetEx.InnerException as CommunicationException;
        if (commEx == null)
        {
          throw targetEx.InnerException; // not a communication exception, throw it
        ...
        {
          Log.Error(string.Format("Invoke<{0}>({1}): exception 2", typeof(TResult), methodInfo), targetEx2);
          throw targetEx2.InnerException; // still failed, throw it
        }
      }
